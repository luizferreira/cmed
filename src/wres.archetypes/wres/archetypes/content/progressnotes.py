...
"""Definition of the ProgressNotes content type
"""
...
from wres.archetypes.interfaces import IProgressNotes
from wres.archetypes.config import PROJECTNAME
from wres.archetypes.content.medicaldocument import MedicalDocument
from wres.archetypes.content.schemas.progressnotes import ProgressNotesSchema

class ProgressNotes(MedicalDocument):
    """ProgressNotes"""
    implements(IProgressNotes)

    meta_type = "ProgressNotes"
    schema = ProgressNotesSchema
    ...
    def at_post_create_script(self):
        self.setTitle('Consulta (Retorno)')
        vt = getToolByName(self, 'vocabulary_tool')
        vocabules = vt.extractFieldValues(self)
...
#
    def __distributeVitalSigns(self, patient, date, source):
        ...
        if vital_signs:
            chart_data = patient.chart_data
            chart_data.add_entry_to('vital_signs', date, source, vital_signs)
    ...
    def __distributeMedications(self, patient, date, source):
        ...
        typed = medications.get('typed', [])
        if typed:
            chart = patient.chart_data
            chart.add_entry_to('medications', date, source, typed)
...
atapi.registerType(ProgressNotes, PROJECTNAME)
