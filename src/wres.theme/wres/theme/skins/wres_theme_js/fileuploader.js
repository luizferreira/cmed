// Communi modified
function treat_same_name_file(name){
    //Treatment
    name = name.toLowerCase()
    for(i = 0; i < name.length; i++){
        //Based on special characteres in ASCII
        if (
           (name.charCodeAt(i) < 46) ||
           (name.charCodeAt(i) == 47) ||
           (name.charCodeAt(i) > 57 && name.charCodeAt(i) < 65) ||
           (name.charCodeAt(i) > 90 && name.charCodeAt(i) < 97) ||
    ...
    name_not_extension = name.split(".")
    extension = "." + name_not_extension.pop()
    name_not_extension = name_not_extension.join("")

    //Check last char equals to - if true remove it
    if(name_not_extension[name_not_extension.length -1] == "-"){
        name_not_extension = name_not_extension.substr(0, name_not_extension.length - 1)
    }
    fileLinks = $(".fileLink")
    for (i = 0; i < fileLinks.length; i++){
        file = fileLinks[i]
        file_name = file.href.split("/").pop()
        file_name_not_extension = file_name.split(".")
        file_extension = "." + file_name_not_extension.pop()
        file_name_not_extension = file_name_not_extension.join("")

        if (file_name_not_extension == name_not_extension && extension == file_extension) {
            new_name = name_not_extension + "-r_2" + extension
            return treat_same_name_file(new_name)
        };
    }
    return name
...
qq.UploadHandlerXhr.prototype = {
    ...
    upload: function(id, params){
        ...
        name = treat_same_name_file(name)
